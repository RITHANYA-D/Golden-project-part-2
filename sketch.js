...
function setup () {
    ...
    arrow = createSprite(700, 550, 100, 100);
    ...
    maze = createSprite(700, 330, 10, 500);
    maze.addImage("maze", mazeImg);
    maze.scale   = 0.61;
    maze.visible = false;
    ...
    restart = createSprite(width/2, height/2 + 200, 50, 50);
    restart.addImage("reset", restartImg);
    restart.scale = 0.6;
    restart.visible = false;

    swiper = createSprite(500, 65, 100, 100);
    ...
    paddle1 = createSprite(800, 02, 900, 05);
    paddle1.visible = false;

    paddle2 = createSprite(800, 655, 900, 05);
...
function obstacle () {
    if(frameCount % 70 === 0) {
        nail = createSprite(width,610,40,10);
        nail.velocityX = -9;
        nail.lifetime  = 150;
        nail.addImage(nailImg);
        nail.scale = 0.1;

        obstaclesGroup.add(nail);
    }
}
